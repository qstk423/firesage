...
BEHAVIOR_MAP = {
    ...
    "损坏挪用消防设施": ["消防设施坏了不修", "损坏消防设施", "挪用消防器材", "擅自拆除", "停用消防设施",
                        ...
                        "喷淋头坏了", "灭火器压力", "掉红区", "不修喷淋"],
    "电动自行车违规停放充电": ["电动车楼道充电", "楼道充电", "电动车进楼", "电瓶车入户", "电动自行车充电",
                              "飞线充电", "电动车停楼道", "电瓶车楼道", "推电动车进电梯", "入户充电",
                              "公共门厅充电", "楼梯间充电", "从窗户飞线", "窗户飞线", "飞线给电动车",
                              "电瓶车插着充电", "楼道里给电瓶车"],
    "违规明火作业": ["电焊", "气焊", "动火", "明火作业", "焊接", "没有动火证", "不给开动火证", "硬焊"],
    "谎报火警": ["谎报火警", "谎报", "报假警", "假火警", "打假119"],
    "无人值班": ["消防控制室无人", "无人值班", "控制室没人", "消控室无人", "没有值班", "减配值班人员",
                  "消控室能不能一个人", "消控室一个人"],
    "值班人员不足": ["只有一名值班", "值班人员不足", "单人值班", "一个人盯着"],
    "未保持完好有效": ["消防设施不符合标准", "配置不符合", "标志不符合", "设施不完好", "没有保持完好"],
    "占用避难层": ["避难层堆放", "占用避难层", "避难层锁闭", "占用避难走道", "避难层当库房", "避难层堆货"],
    "占用电缆井管道井": ["电缆井堆放", "管道井堆物", "占用管井", "井道堆杂物", "电缆井里塞满", "管井塞满"],
    ...
    "常闭防火门敞开": ["常闭防火门开着", "常闭防火门天天开", "防火门敞开", "常闭式防火门开着"],
}
...
OBJECT_MAP = {
    ...
    "消火栓": ["消火栓", "消防栓"],
    "消防设施器材": ["消防设施", "消防器材", "灭火器", "喷淋", "报警系统", "应急照明", "疏散指示标志", "防火门", "常闭防火门", "喷淋头"],
    "消防控制室": ["消防控制室", "消控室", "消控室值班"],
    "避难层": ["避难层", "避难间", "避难走道"],
    "防火间距": ["防火间距"],
    "电动自行车": ["电动自行车", "电动车", "电瓶车"],
