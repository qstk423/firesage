...
- **版本对比**：v1 / v2 / v3 已完成训练与横向评测，**v2 冻结为当前版本，不再训练 v4**（输出格式纪律 100%、basis 引用正确率 78%）；v3 因拒答模板过拟合净回退，保留用于消融
- **推理服务**：`scripts/serve_local_qwen.py`，OpenAI Chat Completions 兼容协议，任何支持该协议的客户端可直接接入
- **一键启停**：`scripts/start_local_stack.ps1` / `stop_local_stack.ps1`（幂等，异常退出后重跑 start 即恢复）

### 4. 五层推理守卫 + 两级管线核验

推理服务五层（`serve_local_qwen.py` 内置，每层失败均兜底不外泄坏输出）：
...
管线两级（`pipeline.py`，生成前拦一道、生成后验一道）：

| 级 | 核验 | 行为 |
|----|------|------|
| 生成前 | 检索门槛 | top1 fused<0.63 且 CrossEncoder<0.60 → 无可靠依据，直接标准拒答（不进 LLM） |
| 生成后 | 原文支撑 | 金额/期限/主体/处罚须能在引用条款原文找到支撑；失败带反馈重试一次，仍失败标准拒答 |
...
v4 方向（**不实施**，版本已冻结）：拒答模板多样化、epochs 降至 1、basis 法条文本截断、应急场景扩写。
...
| 指标 | v0.8.4 | v0.8.5 | **v0.9.0（冻结）** |
|------|--------|--------|--------------------|
| 通过率 | 22/35（62.9%） | 31/35（88.6%） | **34/35（97.1%）** |
| 金标条款命中（有金标 27 题） | — | 22/30 | **21/27** |
| 库外正确拒答（无金标 3 题） | 未区分 | 未区分 | **3/3** |
| 首字响应 TTFT | 未记录 | p50 3.4s | **p50 3.5s / max 5.1s** |

评分口径（v0.9.0 修正）：库外题（知识库未收录）以**正确拒答**为通过，硬答反而失败。唯一未通过的 d27 为模型错引法规名（检索 top1 已命中金标「高层规定·第七条」0.936，模型生成时错引为「消防法·第七条」），引用守卫重试一次仍无效后标准拒答——**宁可拒答，不可编造**，属 LoRA v2 已知局限（见下方版本对比）。
...
│   │   └── graphrag.py         # 图谱构建与检索
│   └── scripts/
│       ├── start_local_stack.ps1   # 一键启动本地栈（8320+8321，幂等）
│       ├── stop_local_stack.ps1    # 一键停止本地栈（含残留进程清理）
...
| 版本 | 要点 |
|------|------|
| **v0.9.0** | **本地演示版本冻结（不再训练 v4）**：评测金标修正（库外题以正确拒答为通过）；一键启停脚本（`start/stop_local_stack.ps1`）；FireEval v0 金标更新（谎报火警、消防验收已入库、guide/chitchat 等价）；README 对齐桌面端定位与 DeepSeek 回退路径 |
...
### 本版发布前验收（2026-09-11，v0.9.0 冻结）

```bash
# 端到端 35 题（本地 LoRA 栈，一键启动后）
powershell -ExecutionPolicy Bypass -File backend\scripts\start_local_stack.ps1
cd backend && ..\venv311\Scripts\python.exe scripts\e2e_local_test.py
# → 34/35 通过（97.1%）；金标命中 21/27；库外拒答 3/3；TTFT p50 3527ms
...
- LoRA 生成在 3B 基座上偶发错引法规名（如「消防法·第七条」实为高层规定第七条），由引用守卫拦截后拒答，未构成错误输出；**冻结版本已知局限，不再训练 v4**
...
- 480 token 输出上限下，个别长职责列举题的 JSON 会触发截断修复守卫（`json_truncation_repair`，修复成功不外泄坏输出）；长答案评测可设 `LOCAL_LLM_MAX_TOKENS=700`
...
6. ~~评测金标修正 + 一键启停 + v0.9.0 冻结发布~~
7. （冻结后不再排期）LoRA v4 方向、半自动 NER 补词典、基层问卷回收
