...
生成侧采用「检索约束下的大模型调用 + 抽取式降级」；**生成式参数高效微调**列入下一阶段，目标是在给定条款内更忠实作答。
...
|------|------|------|
| 路由准确率 | 1.00 | 意图分流正确率 |
| Hit@1 | 0.74 | 首条依据命中目标条款 |
| Hit@3 | 0.87 | 前三条命中 |
| 引用准确率 | 1.00 | 回答引用落在检索依据内 |
| 拒答准确率 | 0.93 | 该拒则拒、不该拒则答 |
| 严重错误率 | 0.00 | 禁止出现的错误结论未出现 |

说明：完整模型链路可用时，检索与生成表现通常高于上述降级基线。汇报时可同时给出「完整链路」与「降级基线」，体现稳健性。
